        ...
        m_activeInterface(0)
{
    ...
    m_currentState = NetworkManager::Device::UnknownState;
    ...
    if (activeInterface()) {
        m_currentState = static_cast<NetworkManager::Device::State>(activeInterface()->state());
    }
...
void NetworkManagerApplet::paintInterface(QPainter * p, const QStyleOptionGraphicsItem *option, const QRect &contentsRect)
{
    ...
    NetworkManager::Device* interface = activeInterface();
    bool useSvg = false;
    if (interface) {
        useSvg = interface->type() == NetworkManager::Device::Wifi || interface->type() == NetworkManager::Device::Ethernet;
    }
...
void NetworkManagerApplet::paintNeedAuthOverlay(QPainter *p)
{
    ...
    /*
    kDebug() << "Painting overlay ...>" << activeInterface()->state();
    */
    if (activeInterface() && activeInterface()->state() == NetworkManager::Device::NeedAuth) {
        //kDebug() << "Needing auth ...>";
...
NetworkManager::Device* NetworkManagerApplet::activeInterface()
{
...
void NetworkManagerApplet::interfaceConnectionStateChanged()
{
    ...
    if (activeInterface()) {
        //kDebug() << "busy ... ?";
        NetworkManager::Device::State state = static_cast<NetworkManager::Device::State>(activeInterface()->state());
        switch (state) {
    ...
    if (interfaces.isEmpty()) {
        ...
        foreach (NetworkManager::Device *iface, interfaces) {
            if (iface->state() != NetworkManager::Device::Unavailable) {
                ...
                RemoteInterfaceConnection *conn = RemoteUtils::connectionForInterface(m_activatables, iface);
                if (conn) {
...
{
    if (activeInterface() && static_cast<NetworkManager::Device::State>(activeInterface()->state()) == NetworkManager::Device::Activated) {
